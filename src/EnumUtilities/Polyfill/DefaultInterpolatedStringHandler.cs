...
namespace System.Runtime.CompilerServices
{
    ...
    public ref struct DefaultInterpolatedStringHandler
    {
...
#if HAS_ENUMTRYFORMAT
                if (typeof(T).IsEnum)
                {
                    int charsWritten;
                    while (!Enum.TryFormat(value, _chars.Slice(_pos), out charsWritten))
                    {
                        Grow();
                    }
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            int arraySize = (int)Math2.Clamp(newCapacity, MinimumArrayPoolLength, int.MaxValue);
