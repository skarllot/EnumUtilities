...
public class EnumNumericFormatterTest
{
        ...
        where T : IMinMaxValue<T>, IBinaryInteger<T>, ISignedNumber<T>
    {
        public static IEnumerable<T> GetRange() =>
            Enumerable
                .Range(0, Math.Max(T.MinValue.ToString()!.Length, T.MaxValue.ToString()!.Length) - 1)
        ...
        where T : IMinMaxValue<T>, IBinaryInteger<T>
    {
        public static IEnumerable<T> GetRange() =>
            Enumerable
                .Range(0, Math.Max(T.MinValue.ToString()!.Length, T.MaxValue.ToString()!.Length))
                .Select(x => T.Parse("1" + new string('0', x), CultureInfo.InvariantCulture))
    ...
    {
        EnumNumericFormatter.GetStringLength(value).Should().Be(value.ToString(CultureInfo.InvariantCulture).Length);
    }
    ...
    public void GetStringLengthInt32(
        [CombinatorialMemberData(nameof(SignedRange<int>.GetRange), MemberType = typeof(SignedRange<int>))] int value
    )
    {
        EnumNumericFormatter.GetStringLength(value).Should().Be(value.ToString(CultureInfo.InvariantCulture).Length);
    }
    ...
    public void GetStringLengthInt64(
        [CombinatorialMemberData(nameof(SignedRange<int>.GetRange), MemberType = typeof(SignedRange<long>))] long value
    )
    {
        EnumNumericFormatter.GetStringLength(value).Should().Be(value.ToString(CultureInfo.InvariantCulture).Length);
    }
